...
bool DirettaSync::open(const AudioFormat& format) {
    ...
    if (m_open && m_hasPreviousFormat) {
        ...
        if (sameFormat) {
            ...
            // DoP-ness is NOT part of sameFormat (DoP and 24-bit PCM look
            // identical there), and this fast path skips configureRingPCM().
            // Keep the silence mode in sync and bump the consumer generation so
            // getNewStream reloads it before the shutdown-silence flush below —
            // otherwise a DoP track would be flushed with 0x00 (audible crack on
            // a DAC locked in DoP), or a PCM track with DoP silence.
            if (m_dopSilence.load(std::memory_order_acquire) != format.isDoP) {
                m_dopSilence.store(format.isDoP, std::memory_order_release);
    ...
    if (m_isDsdMode.load(std::memory_order_acquire)) {
        ...
        if (!configureSinkPCM(format.sampleRate, format.channels, format.bitDepth, acceptedBits)) {
            ...
            // caller reports the failure and moves on to the next track.
            return false;
        }
        ...
        configureRingPCM(format.sampleRate, format.channels, direttaBps, inputBps, format.isCompressed, format.isDoP);
    }
...
void DirettaSync::fullReset() {
...

    {
        ...
        m_dopSilence.store(false, std::memory_order_release);
...
bool DirettaSync::configureSinkPCM(int rate, int channels, int inputBits, int& acceptedBits) {
    ...
    // is never offered FIRST to a 16/24-bit source (DACs such as the TEAC
    // UD-701N announce 32-bit at the Diretta level but are physically 24-bit
    // and play silence/noise) — it is only a last resort on a sink that
    // refuses 24-bit, where it is lossless for us (the ring holds S24 in an
    // S32 container; the 32-bit sink path is a plain copy) and beats a
    // truncation to 16 or a hard failure.
    ...
    for (int bits : order) {
        ...
        if (bits < inputBits) {
            ...
            LOG_INFO("[DirettaSync] Sink refused " << inputBits << "-bit at " << rate
                     << "Hz — using 32-bit (lossless; DACs that announce 32-bit but are "
...
void DirettaSync::configureRingPCM(int rate, int channels, int direttaBps, int inputBps, bool isCompressed, bool isDoP) {
    std::lock_guard<std::mutex> lock(m_configMutex);
    ReconfigureGuard guard(*this);

    // DoP travels through the PCM path (24-bit) but needs valid DoP silence,
    // not 0x00. getNewStream rebuilds the silence pattern when this flips.
    m_dopSilence.store(isDoP, std::memory_order_release);
...
void DirettaSync::configureRingDSD(uint32_t byteRate, int channels) {
    ...
    m_dopSilence.store(false, std::memory_order_release);  // native DSD, not DoP
    m_need24BitPack.store(false, std::memory_order_release);
    m_need16To32Upsample.store(false, std::memory_order_release);
    m_need16To24Upsample.store(false, std::memory_order_release);
    m_channels.store(channels, std::memory_order_release);
...
bool DirettaSync::getNewStream(diretta_stream& baseStream) {
    ...
    // Hot path: use cached values (silence emitted via fillSilence(), which
    // reads m_cachedSilenceByte / the DoP pattern directly)
    ...
    if (m_rebuffering.load(std::memory_order_acquire)) {
        ...
        if (avail >= threshold) {
            ...
            // No iostream here (RT thread): hand the event to the consumer
...
void DirettaSync::applyTransferMode(DirettaTransferMode mode, ACQUA::Clock cycleTime) {
    ...
    // Helper: build minCycle for RANDOM mode
    auto getMinCycle = [this]() {
        return (m_config.cycleMinTime > 0)
            ? ACQUA::Clock::MicroSeconds(m_config.cycleMinTime)
            : ACQUA::Clock::MicroSeconds(333);
    };

    // TargetProfile path: use ProfileMaker for target-adaptive transfer
    if (m_config.targetProfileLimitTime > 0) {
        ...
        switch (effectiveMode) {
            ...
            case DirettaTransferMode::RANDOM: {
                ACQUA::Clock minCycle = getMinCycle();
    ...
    // SelfProfile path: direct Sync method calls (no target adaptation)
    switch (effectiveMode) {
        case DirettaTransferMode::FIX_AUTO:
        ...
        case DirettaTransferMode::RANDOM: {
            ACQUA::Clock minCycle = getMinCycle();
            DIRETTA_LOG("Using Random (min=" << m_config.cycleMinTime
                        << "us, max=" << cycleTime.getMicroSeconds() << "us)");
