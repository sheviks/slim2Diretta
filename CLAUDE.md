...
1. **Diretta format negotiation** (`configureSinkPCM`, ported from DirettaRendererUPnP v2.5.16/PR#95): tries the sink in source-depth-aware order and returns `bool` — an unsupported format fails the track cleanly (`open()` returns `false`) instead of `throw`ing through it. 32-bit input tries 32→24→16; 24-bit input tries 24→32→16; 16-bit input tries 24→16→32 (the historical 24→16, plus 32 as a genuine last resort). 32-bit is never offered *first* to a 16/24-bit source — prevents white noise/silence on DACs that report 32-bit support at the Diretta target level but are physically limited to 24-bit — but it's tried last rather than refused outright, since it's lossless for us (the ring holds S24 in an S32 container) and beats a 16-bit truncation on a sink that refuses 24-bit.
...
`push24BitPacked` uses hybrid S24 auto-detection (MSB-aligned vs LSB-aligned), with `MsbAligned` hint always set for all our decoders (confirmed: every decoder in this codebase produces MSB-aligned data per the paragraph above, so `MsbAligned` is also the correct blind fallback if the hint and detection both come up empty — see `DirettaRingBuffer::clear()`/`resize()` below).

**S24 hint survives `clear()`** (ported from DRUP v2.5.16/PR#95): `clear()` — called on every `resize()` but also whenever playback resumes without a full `open()` — used to reset `m_s24Hint` to `Unknown` along with the rest of the S24 detection state. On a resume during a quiet passage, detection would time out with no hint to fall back on, defaulting to `LsbAligned` — full-scale noise once real audio returned, on a decoder that's actually always MSB-aligned. Fixed: the hint now survives `clear()` (`m_s24PackMode = m_s24Hint` instead of `Unknown`); only `resize()` forgets it, since that's a genuine new-format event and the caller re-sets it via `setS24PackModeHint()` right after. The blind default for a still-missing hint was also flipped from `LsbAligned` to `MsbAligned`, matching what every decoder here actually produces.
...
- **PCM fades around cuts in the middle of the music (v1.4.25)**, ported from DirettaRendererUPnP PR #98 (herisson-88): `getNewStream()` used to jump straight from a music buffer to a zero silence buffer and back on Stop/Pause/track skip — a step in the waveform heard as a click. `diretta/PcmFade.h` (copied unchanged from DRUP) provides 10 ms smoothstep ramps, exact Q16 integer gain, no allocation. `requestShutdownSilence()` arms a fade-out (`m_fadeOutRequest`/`fadeOutBuffer()`) consumed by `getNewStream()` before the shutdown-silence branch; `playOutShutdownSilence()` (now shared by `close()`, `stopPlayback()`, `pausePlayback()` and the same-format quick-resume path in `open()`) also drops the ring as soon as the worker reaches actual silence, instead of leaving it full of stale music a late callback could replay at full level. Fade-in (`m_fadeInRequest`/`fadeInBuffer()`) runs on the first music buffers after `resumePlayback()` and at the end of a rebuffering. DoP is unaffected: it already bypasses this whole mechanism (SDK kept running to preserve the marker stream, see DoP Handling above), and `pcmFadeLayout()` additionally gates on `m_cachedDopSilence` as defense in depth. No `flushForSeek()` exists in this project (unlike DRUP), so that part of the upstream fix does not apply here.
...
- **Underrun/rebuffering events off the RT thread** (ported from DRUP v2.5.16/PR#95): `getNewStream()` (the SDK's real-time worker thread) used to call `LOG_WARN` directly — a `std::cout`, i.e. a mutex, a possible page fault and a `write(2)` to a journal pipe from inside the audio callback. It now only raises bits in an atomic mailbox (`DirettaSync::consumeRtEvents()`, `RT_EVENT_UNDERRUN`/`RT_EVENT_REBUFFER_COMPLETE`, `alignas(64)` on its own cache line); `main.cpp`'s `logRtEvents()` drains it and does the actual logging, called right after every `sendAudio()` in the playback loops (there is no single per-buffer consumer loop here, unlike DRUP's threaded `audioThreadFunc()`, so the drain call is repeated at each of the 6 `sendAudio()` call sites instead of once).
